...
$stmt = $conn->prepare("SELECT * FROM students WHERE id = ? AND center_id = ?");
$stmt->execute([$id, $center_id]);
$student = $stmt->fetch(PDO::FETCH_ASSOC);

if (!$student) {
    header("Location: center_student_list.php");
    exit();
}
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if(isset($_FILES['image']) && $_FILES['image']['error'] == 0) {
        ...
        if(in_array($ext, $allowed)) {
            ...
            $error = "Invalid image format (JPG, JPEG, PNG only)";
        }
    }
    ...
    if(isset($_FILES['id_card']) && $_FILES['id_card']['error'] == 0) {
        ...
        if(in_array($ext, $allowed)) {
            ...
            $error = "Invalid ID card format (JPG, JPEG, PNG, PDF only)";
        }
    }
    ...
    if (!isset($error)) {
        try {
            $sql = "UPDATE students SET student_name = ?, mobile = ?, alt_mobile = ?, email = ?, username = ?, 
                    address = ?, district = ?, pincode = ?, state = ?, country = ?, image = ?, id_card = ? 
                    WHERE id = ? AND center_id = ?";
            $stmt = $conn->prepare($sql);
            $stmt->execute([$student_name, $mobile, $alt_mobile, $email, $username, $address, $district, $pincode, $state, $country, $image_path, $id_card_path, $id, $center_id]);
            ...
            <form method="POST" enctype="multipart/form-data">
                ...
                <div class="row mb-3">
                    <div class="col-md-6">
                        ...
                        <input type="text" name="alt_mobile" class="form-control" value="<?php echo htmlspecialchars($student['alt_mobile']); ?>">
                    </div>
                    <div class="col-md-6">
                ...
                <div class="mb-3">
                    ...
                    <img src="<?php echo htmlspecialchars($student['image'] ?: 'placeholder.jpg'); ?>" class="current-image" alt="Current Image">
                </div>
                <div class="mb-3">
                ...
                <div class="mb-3">
                    <label class="form-label">Current ID Card</label><br>
                    <?php if($student['id_card']): ?>
                        <a href="download.php?file=<?php echo urlencode($student['id_card']); ?>" class="btn btn-sm btn-success">Download Current ID</a>
                    <?php else: ?>
                ...
                <div class="row mb-3">
                    <div class="col-md-4">
                        ...
                        <input type="text" name="district" class="form-control" value="<?php echo htmlspecialchars($student['district']); ?>" required>
                    </div>
                    <div class="col-md-4">
                ...
                <div class="mb-3">
                    ...
                    <input type="text" name="country" class="form-control" value="<?php echo htmlspecialchars($student['country']); ?>" required>
                </div>
